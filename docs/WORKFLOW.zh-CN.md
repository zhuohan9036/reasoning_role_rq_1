...
即使规划与实施发生在不同客户端，工作流仍以角色和文档为基础：

1. **规划对话**讨论科学与工程问题，在对话中记录暂时共识，并且不修改仓库。
2. 动作预告获批后，**Codex 计划生成**创建中英文 `BRIEF`、`SERVER`、`PLAN` 和一个
   `plan.json`，状态为 `awaiting_review`。
3. **人类审核**接受、拒绝或修改该合同。非正式赞同一个想法不构成实施授权。
4. 取得另一项明确批准后，**Codex 实施**只修改获批路径，只运行获批检查。
5. 中英文 **review 交接**记录实际交付、验证、偏离、未执行事项、Git 状态、科学边界和
   下一门禁。

`plan.json` 是机器可读实施合同；中英文文档是面向人类的主要审核路径。若对话发生在
不同客户端，目标 Codex task 必须获得精确 task ID、plan path、获批 revision 或
working-tree state，以及 Git 行为。交接不会携带超出方案所记录状态的授权。
...
对于跨客户端交接，推荐授权语句明确 executor、task、scope 和 Git 行为，例如：

```text
授权 Codex 按已审核方案实施 <task-id>；保持变更未提交，不 push。
```
